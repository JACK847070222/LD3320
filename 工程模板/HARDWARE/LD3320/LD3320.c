...
uint8 LD_Check_ASRBusyFlag_b2(void)
{
	...
	for (j=0; j<10; j++)
	{
		if (LD_ReadReg(0xb2) == 0x21)		//0x21表示空闲
		{
			flag = 1;
			break;
		}
...
{
	LD_WriteReg(0x35, MIC_VOL);		//设置MIC灵敏度，值越大越灵敏
	LD_WriteReg(0x1C, 0x09);
	LD_WriteReg(0xBD, 0x20);
	LD_WriteReg(0x08, 0x01);		//清除FIFO_DATA
	LD3320_delay( 5 );
	LD_WriteReg(0x08, 0x00);		//规定再写入一次0X00
	LD3320_delay( 5);

	if(LD_Check_ASRBusyFlag_b2() == 0)		//判断DSP是否空闲
	{
		return 0;
	}
	...
	LD3320_delay(5);
	LD_WriteReg(0x1C, 0x0b);		//MIC输入通道的ADC激活
	LD_WriteReg(0x29, 0x10);		//允许识别中断
	LD_WriteReg(0xBD, 0x00);   		//启动ASR单元
	return 1;
}
...
//（有声音输入，不论识别成功或失败都有中断）
	...
	{	 
			...
			{
				nAsrStatus=LD_ASR_FOUNDOK; 		//识别到正确选项		
			}
			else
			{
				nAsrStatus=LD_ASR_FOUNDZERO;	//识别错误
			}	
	}
	else
	{
...
static void LD_Init_Common(void)
{
	...
	LD_WriteReg(0x17, 0x4c); 		//休眠DSP
	LD3320_delay(1);
	LD_WriteReg(0xB9, 0x00);
	LD_WriteReg(0xCF, 0x4F); 
	LD_WriteReg(0x6F, 0xFF); //手册没找到
}
